...
func (h *Handler) sendStartMessage(bot *telego.Bot, chatID int64) {
	err := h.service.SendMessage(bot, chatID, messages.MsgHello)
	if err != nil {
		logrus.Errorf("handler sendStartMessage: cant't send start message %v", err)
	}
}

func (h *Handler) sendHelpMessage(bot *telego.Bot, chatID int64) {
	err := h.service.SendMessage(bot, chatID, messages.MsgHelp)
	if err != nil {
		logrus.Errorf("handler sendHelpMessage: cant't send help message %v", err)
	}
}
