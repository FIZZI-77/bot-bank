...
func (h *Handler) sendMoney(username string, amount float64, recipient string, isCommandCorrect bool, bot *telego.Bot, chatID int64) {
	...
	if err != nil {
		logrus.Errorf("bank-operation hendler: sendMoney() :cant't send money: %s", err.Error())
		if sendErr := h.service.SendMessage(bot, chatID, messages.MsgErrorSend); sendErr != nil {
			logrus.Errorf("bank-operation hendler: sendMoney(): cant't send error Send  message %v", sendErr)

		}
		return
	}
	if err := h.service.SendMessage(bot, chatID, messages.MsgSendMoney); err != nil {
		logrus.Errorf("bank-operation hendler: sendMoney() :cant't send Send message %v", err)
	}
}

func (h *Handler) topUpMoney(username string, amount float64, isCommandCorrect bool, bot *telego.Bot, chatID int64) {
	if !isCommandCorrect {
		return
	}
